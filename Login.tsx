...
const Login: React.FC<Props> = ({ onLogin, darkMode }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (mode === 'login') {
        // RÈGLE STRICTE : Seul 'akadio' est Administrateur
        if (lowerUser === 'akadio' && password === 'admin') {
          onLogin('akadio', 'Admin');
          return;
        }

        // Pour tout autre utilisateur, accès en tant qu'Agent
        if (username.length >= 4 && password.length >= 4) {
          // Tous les autres utilisateurs sont AGENT par défaut
          onLogin(username, 'Agent');
        } else {
          setError('Identifiant ou mot de passe incorrect (min. 4 car.).');
          setLoading(false);
        }
      } else {
        // Mode Inscription - Toujours Agent par défaut
    ...
    <div className={`min-h-screen flex items-center justify-center p-4 transition-colors duration-500 ${darkMode ? 'bg-slate-950' : 'bg-slate-50'}`}>
      ...
      <div className={`w-full max-w-[460px] z-10 p-8 lg:p-12 rounded-[40px] border shadow-2xl transition-all duration-500 animate-in fade-in zoom-in-95 ${darkMode ? 'bg-slate-900/80 border-slate-800 shadow-black' : 'bg-white/90 border-slate-100 shadow-slate-200'}`}>
        ...
        <div className={`mt-10 p-4 rounded-2xl border border-dashed text-center ${darkMode ? 'border-slate-700 bg-slate-800/50' : 'border-slate-200 bg-slate-50'}`}>
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Accès Super Administrateur Unique</p>
          <div className="flex flex-col gap-1">
             <p className="text-[10px] font-black text-red-500">ID: akadio | PASS: admin</p>
             <p className="text-[8px] font-black text-slate-500 uppercase">Tous les autres comptes sont des AGENTS</p>
          </div>
        </div>
      </div>
    </div>
